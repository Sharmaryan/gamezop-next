...
export const GameCard = ({
  ...
  return (
    <div className="flex flex-col w-40 border-2 border-primary px-5 py-3 rounded-xl gap-5 bg-background_primary md:w-60 xl:w-[300px]">
      ...
      <Link href={url ?? "#"} className="relative h-20 w-full md:h-40 xl:h-60" aria-label="game image">
        <Image
          src={assets.cover}
          alt=""
          fill
      ...
      <div className="flex mt-auto">
        {gamePreviews?.en && (
          <Tooltip tooltipText="Watch Video">
            <Link href={gamePreviews.en} className="text-secondary" aria-label="youtube">
              <div>
                <FaYoutube size={15} className="text-warning md:hidden" />
                <FaYoutube size={20} className="text-warning hidden md:block xl:hidden" />
        ...
        {url && (
          <Tooltip tooltipText="Play">
            <Link href={url} className="text-secondary" aria-label="gaming">
              ...
              <FaGamepad size={20} className="text-success hidden md:block xl:hidden" />
